...
class ListLoadingScreen extends StatefulWidget {
  ListLoadingScreen({this.home, required this.listMemory});
  final home;
...
class _ListLoadingScreenState extends State<ListLoadingScreen> {
  ...
  void loding() async {
    ...
    for (var i = 0; i < widget.listMemory.length; i++) {
      GetListData getListData = GetListData(cityName: widget.listMemory[i], weatherIcon: '');
  ...
  class GetListData {
    GetListData({required this.cityName, this.temp, required this.weatherIcon,this.wind});
